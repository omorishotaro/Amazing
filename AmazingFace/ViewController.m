...
- (void)viewDidLoad {
    ...
    NSLog(@"positon %f, %f, %f", node.position.x, node.position.y, node.position.z);
...
//    }
    ...
    // create new geometry
    SCNText *text = [SCNText textWithString:@"text" extrusionDepth:2];
    ...
    [textNode.geometry setMaterials:materials];
    
    // update geometrt
    [[[node childNodes] objectAtIndex:0] setGeometry:textNode.geometry];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    //SpeachTextGetter *speachTextGetter = [SpeachTextGetter getInstance];
    //[speachTextGetter readyForSTT];
    //[speachTextGetter startListening];
}
...
//    [node setPosition:tranVec];
    ...
    [node setPosition:tranVec];
    NSString *message = [[SpeachTextGetter getInstance] getPreText];
    SCNText *text = [SCNText textWithString:message extrusionDepth:2];
    SCNNode *textNode = [SCNNode node];
    textNode.geometry = text;
    textNode.position = SCNVector3Make(0.000000, 0.000000, 0.014795);
    [scene.rootNode addChildNode:textNode];
